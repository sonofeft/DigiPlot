...
import numpy as np
from digiplot.InterpProp_scipy import InterpProp
...
class _auto_detect(_Dialog):
    ...
    def body(self, master):
        ...
        self.make_Label_4( self.Frame_1 )              #       Label: Max Filter Size : at Frame_1(6,1)
        ...
        self.num_points = 10 # number of points to generate
        self.max_filter_size = 1 # used in ImageFilter.MaxFilter (MUST BE ODD)
        self.opacity = 99 # for points on drawing
    ...
    def build_PA_img(self):
        ...
        if self.max_filter_size > 1:
            gray = gray.filter( ImageFilter.MaxFilter(self.max_filter_size) ) 
        
        gblur = gray.filter( ImageFilter.GaussianBlur(radius = self.blur_radius) ) 
        
        #pLL = self.pointL[:] # copy of reference points
        ...
        #    target = gblur.convert("RGBA")
        #else:
        #    target = gray.convert("RGBA")
        target = Image.new('RGBA', (w,h), (0,0,0,0))
        target_mask = Image.new('L', (w,h), 0)
        
        draw = ImageDraw.Draw( target )
        draw_mask = ImageDraw.Draw( target_mask )
        ...
        for i in range(101):
            #fi = fi_min + i*(fi_max-fi_min)/100.0
            s = i*s_total / 100.0
            fi = fi_sterp( s )
        ...
        draw.line( ijL, width=3, fill='red' )
        draw_mask.line( ijL, width=3, fill=self.opacity )
        ...
        for (x,y,fi,fj) in pLL:
            ...
            f_jpos = h * fj
            j = int( f_jpos )
            draw.rectangle(((i-4,j-4),(i+4,j+4)), fill ="blue", outline ="cyan") 
            draw_mask.rectangle(((i-4,j-4),(i+4,j+4)), fill=self.opacity, outline =self.opacity) 
        ...
        for i in range(Npts + 1):
            ...
            #fi = fi_min + i*(fi_max-fi_min)/ float(Npts)
            fj = fjterp( fi )
            ...
            draw.line( ((i1,j1),(i2,j2)), width=3, fill='magenta' )
            draw_mask.line( ((i1,j1),(i2,j2)), width=3, fill=self.opacity )
            #draw.rectangle(((i1-4,j1-4),(i1+4,j1+4)), fill ="blue", outline ="cyan") 
            
            # get 1D color array for edge-finding. (FROM gblur)
            def get_ave_color( fi, fj ):
                i1 = int(  fi * w )
                j1 = int(  fj * h )
                c11 = gblur.getpixel( (i1,j1) )
                return c11
            ...
            if iave<Nsteps/4 or iave>Nsteps*3/4:
                ...
                f_jpos = h * fj
                j = int( f_jpos )
                draw.rectangle(((i-4,j-4),(i+4,j+4)), fill='green', outline ="green") 
                draw_mask.rectangle(((i-4,j-4),(i+4,j+4)), fill=self.opacity, outline =self.opacity) 
        ...
        if self.Checkbutton_2_StringVar.get() == 'yes':
            show_img = gblur.convert("RGBA")
        else:
            show_img = gray.convert("RGBA")
        show_img.paste(target, mask=target_mask)
    ...
    def make_Label_2(self, frame):
        ...
        self.Label_2 = Label( frame , text="#Generated Points", width="15", anchor="w", justify="left")
    ...
    def make_Label_4(self, frame):
        """       Label: Max Filter Size : at Frame_1(6,1)"""
        self.Label_4 = Label( frame , text="Max Filter Size", width="15", anchor="w", justify="left")
    ...
    def make_Spinbox_2(self, frame):
        ...
        self.Spinbox_2_StringVar.set("10")
    ...
    def make_Spinbox_3(self, frame):
        ...
        self.Spinbox_3.configure(textvariable=self.Spinbox_3_StringVar, from_="1", to="15")
        self.Spinbox_3_StringVar.set("1")
    ...
    def make_Spinbox_4(self, frame):
        ...
        self.Spinbox_4_StringVar.set("99")
    ...
    def Spinbox_3_StringVar_Callback(self, varName, index, mode):
        ...
        if new_val %2 == 1:
            self.max_filter_size = new_val
        elif new_val > self.max_filter_size and new_val < MAX_SIZE:
            self.max_filter_size = new_val + 1
            self.Spinbox_3_StringVar.set( new_val+1 )
        elif new_val < self.max_filter_size and new_val > 0:
            self.max_filter_size = new_val - 1
            self.Spinbox_3_StringVar.set( new_val-1 )
        elif new_val >= MAX_SIZE:
            self.max_filter_size = MAX_SIZE
            self.Spinbox_3_StringVar.set( MAX_SIZE )
            
        #print('self.max_filter_size =',self.max_filter_size)
        self.fill_canvas()
