"""A setuptools based setup module for DigiPlot

See:
http://digiplot.readthedocs.org/en/latest/
https://github.com/sonofeft/DigiPlot

If installing from source, then
the best way to install DigiPlot is to use pip after navigating to the source directory::
...
target_file = path.join( here, 'digiplot','_version.py')
...
setup(
    ...
    description = 'A tkinter code that helps users digitize curves from plot images.',
    long_description = long_description,

    # The project's main homepage.
    url='http://digiplot.readthedocs.org/en/latest/',
    download_url='https://github.com/sonofeft/DigiPlot',
    ...
    test_suite='digiplot.tests', # allows "setup.py test" to work
    ...
    entry_points = {
        'console_scripts': [
            'digiplot=digiplot.tk_digiplot:main',
        ],
    },
)